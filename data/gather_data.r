...
### Individual fish fixes (flags, sheds, clumpped data, etc.) ----------------------------------------------------------

# BT 149.400 209 - Lots of clumping in 2024 near dam, thinning out
slimmed_5_209 <- filter(ind_d, Ch == 5 & Code == 209, year(Datetime) == 2024, haul == "Movement") %>%
  mutate(Month = as.numeric(format(Datetime, "%m"))) %>% filter(Month %in% c(5:7)) %>% 
  mutate(Date = format(Datetime, "%Y-%m-%d")) %>%  group_by(Date, haul) %>% slice_head() %>%  
  ungroup() %>% select(-Month, -Date)

remove_5_209  <- ind_d %>% mutate(Month = as.numeric(format(Datetime, "%m"))) %>% 
  filter(Ch == 5 & Code == 209, year(Datetime) == 2024,  haul == "Movement", Month %in% c(5:7)) %>% 
  anti_join(slimmed_5_209)

#### Filtering out tags ...............................................
filtered_ind <- ind_d %>% 
  filter(!(Ch == 3 & Code == 627 & Datetime > ymd_hms('2022-11-08 08:53:57', tz="UTC"))) %>%  #Possible shed
  filter(!(Ch == 5 & Code == 120 & Datetime > ymd_hms('2023-05-15 00:27:05', tz="UTC"))) %>%  #Possible shed
  anti_join(remove_5_209)

#QAQC
nrow(ind_d) - 2 - nrow(remove_5_209) == nrow(filtered_ind) #SHOULD BE TRUE
